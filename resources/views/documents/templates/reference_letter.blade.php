...
<div style="padding-bottom:55px">
    ...
    <div style="padding:24px 30px 0">
        ...
        <p>
            We write to introduce <strong>{{ $customer->first_name }} {{ $customer->last_name }}</strong>,
            a customer of <strong>{{ $tenant?->name ?? config('app.name') }}</strong> in good standing.
            This reference letter is provided at the specific request of the above-named customer and
            is strictly for the purpose stated herein.
        </p>

        {{-- Reference standing box --}}
        <table width="100%" cellpadding="0" cellspacing="0"
               style="margin:18px 0; border:1.5px solid #7c3aed; border-radius:6px; background:#f5f3ff">
            <tr>
                <td style="width:56px; text-align:center; vertical-align:middle; padding:18px 10px">
                    <table cellpadding="0" cellspacing="0"
                           style="margin:0 auto; width:38px; height:38px; background:#7c3aed; border-radius:19px">
                        <tr>
                            <td style="text-align:center; vertical-align:middle; font-size:9px; font-weight:900; color:#fff; line-height:1.2">
                                REF
                            </td>
                        </tr>
                    </table>
                ...
                <td style="padding:18px 14px 18px 0; vertical-align:middle">
                    <div style="font-size:13px; font-weight:700; color:#5b21b6; margin-bottom:3px">
                        CUSTOMER IN GOOD STANDING
                    </div>
                    <div style="font-size:10px; color:#6d28d9">
                        Satisfactory banking relationship confirmed as at {{ now()->format('d F Y') }}
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table width="100%" cellpadding="0" cellspacing="0"
            ...
            <tr style="background:#ffffff">
                <td class="info-label">Customer Since</td>
                <td class="info-value">{{ $account->created_at?->format('F Y') ?? 'N/A' }}</td>
            </tr>
            <tr style="background:#f8f9fc">
                <td class="info-label">Account Status</td>
                <td class="info-value status-ok">ACTIVE — IN GOOD STANDING</td>
            </tr>
            <tr style="background:#ffffff">
                <td class="info-label" style="border-bottom:none">KYC Status</td>
                <td class="info-value" style="border-bottom:none">{{ strtoupper(str_replace('_', ' ', $customer->kyc_tier ?? 'Level 1')) }}</td>
            </tr>
        </table>

        <p>
            Based on our records, <strong>{{ $customer->first_name }} {{ $customer->last_name }}</strong>
            has maintained a satisfactory banking relationship with {{ $tenant?->name ?? config('app.name') }}.
            We are pleased to provide this reference in good faith and without any liability on the part
            of the bank arising from reliance placed on this letter.
        </p>

        <p>
            This reference is provided solely for the purpose requested by the customer and should not
            be relied upon for any other purpose without the express written consent of
            {{ $tenant?->name ?? config('app.name') }}.
        </p>
